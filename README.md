...
| `working` (over 1h) | <img src="src/squid_pet/frontend/sprites/pancake_flip_1.png" width="56" alt="working for over an hour"> | The **same** `working` state, drawn differently once the current continuous working stretch passes `long_working_threshold_sec` (default 1h) | Your agent has been grinding on one thing for an hour — she puts on a chef's hat and starts flipping pancakes, because this one's really cooking |
