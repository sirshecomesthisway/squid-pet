...
Indigo runs on macOS and renders a 200×220 always-on-top, transparent, frameless
...
- Making the transparent portion of the window genuinely click-through so it
  doesn't block whatever Confluence/IDE window is behind Indigo
...
- Drag anywhere on Indigo's body to move her; right-click to snap to corners.
...
- AI inside Indigo (she only reflects state; she doesn't generate replies).
...
Indigo must not require any change to Code Puppy itself.
...
flowchart TD
    ...
    subgraph FS ["Filesystem signals"]
      ...
      STATE[~/.indigo-pet/state.json]
      POS[~/.indigo-pet/position.json]
    end
...
| State detection is heuristic, not authoritative | Acceptable — Indigo is ambient. False "thinking" during a long compile is fine |
| Multi-monitor + dragging Indigo to secondary display | NSEvent.mouseLocation returns coords in primary-screen space; cross-display drag is best-effort |
